...
DEST_DIR="$(cd "$SCRIPT_DIR/.." && pwd)/assets/samples/Sponza"
...
echo ""
echo "fetch_sponza: $DONE downloaded, $SKIPPED skipped, $FAILED failed (total=$TOTAL)"
if [[ $FAILED -gt 0 ]]; then
    echo "Some files failed. Re-run the script to retry." >&2
    exit 1
fi
echo "Sponza assets ready at: $DEST_DIR"
