...
// external sample — proves the library is independently linkable with no
// hello_engine dependency.
//
...
//   * Constructs a cd::editor::panel::animator::Animator and seeds it with
//     multiple clips + a mid-point playback position so all sections of the
//     panel (clip browser, timeline scrubber, play/loop strips) render with
//     non-trivial state on the first frame.
//   * Render loop:
//       batcher.begin_frame()
...
int main(int argc, char** argv)
{
    ...
    // ---- Animator panel ------------------------------------------------------
    //
    // Seed with three clips and a mid-point playback position so every visual
    // section of the panel (clip browser, timeline scrubber, play/loop strips)
    // has non-trivial state from the very first frame.
    cd::editor::panel::animator::Animator panel;
    ...
    panel.add_clip(kClipIdle);
    panel.add_clip(kClipWalk);
    panel.add_clip(kClipAttack);
    panel.set_clip(kClipWalk);          // highlight the second clip
    panel.set_duration(2.4F);           // 2.4-second walk cycle
    panel.set_time(1.2F);               // scrubber at 50 %
    panel.set_playing(true);
    panel.set_looping(true);

    std::printf("[hello_animator] Animator seeded:\n"
                "  clip_count = %zu\n"
                "  active_clip = %u\n"
                "  duration    = %.2f s\n"
                "  time        = %.2f s\n"
                ...
                panel.clip_count(),
                panel.clip_id(),
                static_cast<double>(panel.duration()),
                static_cast<double>(panel.time()),
                panel.is_playing() ? "true" : "false",
                panel.is_looping() ? "true" : "false");
    std::fflush(stdout);
    ...
    {
        std::fprintf(stderr, "[hello_animator] platform::create_window failed.\n");
        return 1;
    }
    ...
    {
        std::fprintf(stderr, "[hello_animator] create_vulkan_device failed.\n");
        return 2;
    }
    ...
    {
        std::fprintf(stderr, "[hello_animator] Renderer::create failed.\n");
        return 3;
    }
    ...
    {
        std::fprintf(stderr,
                     "[hello_animator] Submitter::create_with_inline_shader failed.\n");
        return 4;
    }
    auto& submitter = *sub_r;
    std::printf("[hello_animator] Submitter ready (Route B / inline GLSL). ESC to exit.\n");
    std::fflush(stdout);
    ...
    while (true)
    {
        ...
        //
        // The full-window rect is passed to Animator::draw() so the panel
        // fills the entire framebuffer (no DockSpace required for this minimal
        // sample).
        ...
        if (!frame_r.has_value())
        {
            ...
            std::fprintf(stderr, "[hello_animator] begin_frame failed.\n");
            return 5;
        }
        auto& frame = *frame_r;
        ...
        if (!end_r.has_value())
        {
            ...
            std::fprintf(stderr, "[hello_animator] end_frame failed.\n");
            return 6;
        }
    ...
    std::printf("[hello_animator] clean exit (%u frames; batcher cmds last=%zu).\n",
