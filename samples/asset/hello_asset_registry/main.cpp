...
int main()
{
    ...
    src->put_text("data/config.json", R"({"app":"chromodynamic","version":1,"vsync":true})");
    ...
    if (!cfg_id)
    {
        std::printf("load json failed\n");
        return 1;
    }
    auto wav_id = registry.load("wav", "audio/empty.wav");
    if (!wav_id)
    {
        std::printf("load wav failed\n");
        return 2;
    }
    ...
    {
        std::printf("missing app field\n");
        return 4;
    }
    std::printf("app name : %s\n", (*name)->as_string().c_str());

    std::printf(
        "wav meta : %u Hz, %u channels, %u bits, %zu frames\n",
        wav->wav().sample_rate,
        wav->wav().channels,
        wav->wav().bits_per_sample,
        wav->wav().frame_count()
    );

    // 5. Idempotency check — second load returns the cached id.
    auto wav_id2 = registry.load("wav", "audio/empty.wav");
    if (!wav_id2 || *wav_id2 != *wav_id)
    {
        std::printf("cache miss on second load\n");
        return 5;
    }
