...
using cd::post_bloom::CpuImage;
using cd::post_bloom::downsample_box;
using cd::post_bloom::Settings;
using cd::post_bloom::soft_threshold;
using cd::post_bloom::upsample_tent;
...
{
    EXPECT_FALSE(cd::post_bloom::kDownsampleCS.empty());
    EXPECT_FALSE(cd::post_bloom::kUpsampleCS.empty());
    EXPECT_FALSE(cd::post_bloom::kPrefilterCS.empty());
    // Each kernel must declare the local workgroup size (sanity).
    EXPECT_NE(cd::post_bloom::kDownsampleCS.find("local_size_x"),
              std::string_view::npos);
    EXPECT_NE(cd::post_bloom::kUpsampleCS.find("local_size_x"),
              std::string_view::npos);
}
