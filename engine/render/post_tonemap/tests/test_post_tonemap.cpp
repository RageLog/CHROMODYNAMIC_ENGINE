...
using cd::post_tonemap::agx;
using cd::post_tonemap::hable;
using cd::post_tonemap::hill_aces;
using cd::post_tonemap::narkowicz;
using cd::post_tonemap::Operator;
using cd::post_tonemap::tonemap;
...
TEST(PostTonemap, GlslDispatcherReturnsNonEmptyForEveryOperator)
{
    for (auto op : { Operator::kNarkowicz, Operator::kHill,
                     Operator::kHable, Operator::kAgx })
    {
        const auto src = cd::post_tonemap::glsl_for(op);
