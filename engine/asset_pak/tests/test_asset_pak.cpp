...
{
    cd::asset_pak::PakBuilder b;
    auto bytes = b.build();
    EXPECT_GE(bytes.size(), cd::asset_pak::kPakHeaderSize);

    cd::asset_pak::Pak pak;
...
TEST(AssetPak, BadMagicRejected)
{
    ...
    EXPECT_EQ(r.error().code,
              static_cast<std::uint32_t>(cd::asset_pak::pak_errors::Code::kMagicMismatch));
}
...
TEST(AssetPak, ForEachIteratesAllEntries)
{
    ...
    int names_seen = 0;
    pak.for_each([&](std::string_view name, cd::asset_pak::PakBlobView blob) {
        ++names_seen;
