// =============================================================================
// CHROMODYNAMIC — cd/asset/image/AssetLoader.hpp
//
...
namespace cd::asset::image
{

class ImageAsset final : public cd::asset::IAsset
{
public:
    explicit ImageAsset(Image i) noexcept
        : image_ { std::move(i) }
    {
    }
...
public:
    ...
    decode(std::span<const std::byte> bytes, std::string_view /*path_hint*/) override
    {
        ...
        auto r = cd::asset::image::load_image_from_memory(std::span<const std::uint8_t> { u8, bytes.size() });
        if (!r.has_value())
            return std::unexpected(r.error());
        return std::unique_ptr<cd::asset::IAsset> { std::make_unique<ImageAsset>(std::move(*r)) };
    }
};
