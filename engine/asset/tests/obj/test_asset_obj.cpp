...
{
    auto r = cd::asset::obj::parse_obj(kTriangle);
...
{
    auto r = cd::asset::obj::parse_obj(kQuadWithUvAndNormal);
    ASSERT_TRUE(r.has_value()) << r.error().message;
...
{
    auto r = cd::asset::obj::parse_obj(kCube);
    ASSERT_TRUE(r.has_value()) << r.error().message;
...
f -3 -2 -1
    ...
    auto r = cd::asset::obj::parse_obj(neg);
    ASSERT_FALSE(r.has_value());
    EXPECT_EQ(r.error().code, static_cast<std::uint32_t>(cd::asset::obj::obj_errors::Code::kUnsupported));
}
...
TEST(AssetObj, EmptyInputReturnsInvalidArgument)
{
    ...
    ASSERT_FALSE(r.has_value());
    EXPECT_EQ(r.error().code, static_cast<std::uint32_t>(cd::asset::obj::obj_errors::Code::kInvalidArgument));
}

TEST(AssetObj, MissingFileReturnsFileNotFound)
{
    auto r = cd::asset::obj::load_obj("c:/definitely/does/not/exist.obj");
    ASSERT_FALSE(r.has_value());
    EXPECT_EQ(r.error().code, static_cast<std::uint32_t>(cd::asset::obj::obj_errors::Code::kFileNotFound));
}

// ----- AssetLoader adapter -----

#include <cd/asset/obj/AssetLoader.hpp>
