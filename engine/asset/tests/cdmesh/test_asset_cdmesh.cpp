...
TEST(CdMesh, SaveLoadRoundtripStdVertex)
{
    ...
    std::vector<cd::asset::cdmesh::CdVertexStd> verts = {
    ...
    d.vertices = std::span<const std::uint8_t> { reinterpret_cast<const std::uint8_t*>(verts.data()),
                                                 verts.size() * sizeof(cd::asset::cdmesh::CdVertexStd) };
    ...
    d.vertex_stride = sizeof(cd::asset::cdmesh::CdVertexStd);
    d.index_stride = 4;
    d.bbox_min = { -0.5F, -0.5F, 0.0F };
    d.bbox_max = { 0.5F, 0.5F, 0.0F };

    ASSERT_TRUE(cd::asset::cdmesh::save(g.path.string(), d).has_value());

    auto loaded = cd::asset::cdmesh::load(g.path.string());
    ASSERT_TRUE(loaded.has_value()) << loaded.error().message;
    EXPECT_EQ(loaded->vertex_count, 3U);
    EXPECT_EQ(loaded->index_count, 3U);
    EXPECT_EQ(loaded->vertex_stride, sizeof(cd::asset::cdmesh::CdVertexStd));
...
TEST(CdMesh, RoundtripU16Indices)
{
    ...
    std::vector<cd::asset::cdmesh::CdVertexStd> verts(4);
...
{
    auto r = cd::asset::cdmesh::load("c:/no/such/path.cdmesh");
    ASSERT_FALSE(r.has_value());
    EXPECT_EQ(r.error().code, static_cast<std::uint32_t>(cd::asset::cdmesh::cdmesh_errors::Code::kFileNotFound));
}
...
TEST(CdMesh, BadMagicReturnsMagicMismatch)
{
    ...
    auto r = cd::asset::cdmesh::load(g.path.string());
    ASSERT_FALSE(r.has_value());
    EXPECT_EQ(r.error().code, static_cast<std::uint32_t>(cd::asset::cdmesh::cdmesh_errors::Code::kMagicMismatch));
}
...
TEST(CdMesh, TruncatedPayloadReturnsCorrupt)
{
    ...
    {
        std::ofstream f(g.path, std::ios::binary);
        f.write(cd::asset::cdmesh::kMagic.data(), 4);
        ...
        w32(cd::asset::cdmesh::kFormatVersion);
        w32(0);      // flags
    ...
    auto r = cd::asset::cdmesh::load(g.path.string());
    ASSERT_FALSE(r.has_value());
    EXPECT_EQ(r.error().code, static_cast<std::uint32_t>(cd::asset::cdmesh::cdmesh_errors::Code::kCorrupt));
}
...
TEST(CdMesh, RejectsZeroVertexCountOnSave)
{
    ...
    d.vertex_stride = 32;
    d.index_stride = 4;
    auto r = cd::asset::cdmesh::save("anything.cdmesh", d);
    ASSERT_FALSE(r.has_value());
    EXPECT_EQ(r.error().code, static_cast<std::uint32_t>(cd::asset::cdmesh::cdmesh_errors::Code::kInvalidArgument));
}

TEST(CdMesh, RejectsBadIndexStride)
{
    std::vector<cd::asset::cdmesh::CdVertexStd> v(1);
    std::vector<std::uint8_t> i(1);
    cd::asset::cdmesh::SaveDesc d {};
    d.vertices = std::span<const std::uint8_t> { reinterpret_cast<const std::uint8_t*>(v.data()),
                                                 v.size() * sizeof(cd::asset::cdmesh::CdVertexStd) };
...
#include <cd/asset/cdmesh/AssetLoader.hpp>
...
std::vector<std::byte> build_minimal_cdmesh_bytes()
{
    ...
    auto save_r = cd::asset::cdmesh::save(p.string(), d);
    if (!save_r)
        return {};
