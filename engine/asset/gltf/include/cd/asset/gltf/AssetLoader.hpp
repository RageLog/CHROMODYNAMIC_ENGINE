// =============================================================================
// CHROMODYNAMIC — cd/asset/gltf/AssetLoader.hpp
//
...
// stay with `cd::asset::gltf::load_gltf(path)` until a VFS-backed
...
public:
    ...
    decode(std::span<const std::byte> bytes, std::string_view /*path_hint*/) override
    {
        ...
        auto r = cd::asset::gltf::load_gltf_from_memory(u8, bytes.size());
        if (!r.has_value())
