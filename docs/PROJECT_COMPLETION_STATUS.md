...
| cd::io | 85 | production | 933 LOC/8 hdrs (BinaryStream/BitStream/Endian/Crc32/Framing/ByteBuffer/Hex/Path); 42 gtests |
| cd::log | 85 | production | 1k LOC/10 hdrs (Console/Json loggers, RingBufferSink, AuditTrail, Format, PanicDump); 24 gtests |
...
| cd::game::settings | 88 | production | 389 src; INI key=value w/ comment preservation + heuristic typed parser via charconv; 12 gtests/64 asserts |
| cd::game::ai_director | 85 | production | 165 src; intensity pacing + encounter-template select + saturating score; 17 gtests/46 asserts |
| cd::ai::pathfinding | 85 | production | 541 src; A* navmesh (Hart/Nilsson) + Triangulator (Recast/Snook); 16 gtests/43 asserts |
...
| cd::game::dialog_tree | 85 | production | 229 src; BG3-style condition-routed graph + kCondition chaining + O(1) hash index; 8 gtests/39 asserts |
| cd::game::fsm | 85 | production | header-only by design (478-LOC Harel statechart: flat+hierarchical + shallow/deep history); .cpp is ABI anchor; 12 gtests/44 asserts |
...
| cd::game::save_compression | 85 | production | 374 src; custom RLE codec + optional LZ4 block (gated); 13 gtests/43 asserts |
...
| cd::gluon | 90 | production | Shader-MODULE lib: 21 embedded .glsl modules (~1.5k LOC SOTA-cited BRDF/tonemap/IBL/LTC) + 47-LOC resolver; 24 tests; consumed by all 3 RHI backends |
| cd::material | 88 | production | 1035 src + 2.6k hdr (StandardPbr/LitPbr/Skinned/Sky/BrdfLut/RtClosestHit); 51 tests incl RT chrome-sphere-reflects-geometry + alpha + MR contract |
| cd::shader | 88 | production | 756 src; CachedCompiler + real glslang-backed GlslangCompiler + FileWatcher hot-reload; 32 tests incl live-edit; Slang path intentional gate-stub |
...
| cd::debug_line | 88 | production | header-only 363 LOC; RHI-independent CPU line-batch (AABB/OBB/frustum/circle → kLineList stream); 20 tests; bgfx/Bevy cited |
...
| Lib | % | Tier | Basis |
|---|---|---|---|
| cd::ecs | 88 | production | ~2.1k LOC hdr-inline (sparse-set World 346 + Scheduler 407 + ArchetypeWorld 543 side-layer + SystemGraph); 179 .cpp; 77 gtests/5 files; dual storage, no stubs |
| cd::scene | 88 | production | ~2.1k LOC/21 hdrs (Scene graph + Serializer 314 + BinarySerializer 259 + Frustum/SpatialHash/Light/Trigger); 132 .cpp; 86 gtests; rich, no stubs |
...
| Lib | % | Tier | Basis |
|---|---|---|---|
| cd::ui_layout | 88 | production | 946 src (Flex 410 + Constraint 536); full Yoga-shape solver (grow/shrink/6 justify modes/align/gap/clamp/nested) + Cassowary constraint; 23 gtests; incremental compute_main/cross reserved no-op |
