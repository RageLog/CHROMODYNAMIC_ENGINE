...
- **C-PARITY-BROADEN** | ADD 3-4 scenes to the Vk-vs-D3D12 pixel-parity (cull=kBack winding, true non-opaque alpha blend, sRGB target, MRT, compute-write-then-sample) — D16 is one narrow scene (cull NONE/opaque/no-MRT/no-MSAA). **DONE** — alpha/sRGB/MRT shipped (Wave 3d); cull=kBack/kFront cross-backend FACING parity **RESOLVED (parity1224)**: the Wave-3d deferral (Vk vs D3D12 classified the same winding with opposite facing) was empirically traced to a REAL D3D12 bug — the phase1204 `FrontCounterClockwise` inversion — removed; locked by `CrossBackendCullFacingRealGeom` (real geometry, both cull modes, revert-proof) + corrected `test_d3d12_face_cull_parity`. NO deferral remains. | `tests/test_backend_pixel_parity.cpp` | **M** | windows-now (DONE)
