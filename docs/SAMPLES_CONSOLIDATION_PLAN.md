...
- **Batch 4 (endgame audit, 2026-06-13 — analyst kanıt-tabanlı):**
  ...
  - **NEEDS-PORT (5 binary, ODAKLI GELECEK SESSION — gerçek entegrasyon,
    ...
    | hello_ui | ui_layout+ui_widgets+ui_renderer_rhi::Submitter | hello_engine overlay | orta-yüksek | golden-gated cd::ui::Submitter (Job B/A altyapısı) | **DONE phase1177 (FOLD — ADR-20260614 §3.4 Job C; Button/Slider/Label/Flex demo → golden-gated "UI Widgets Demo" R-Showcase panel; RE-USES the BehaviorDesigner Submitter overlay (no second Submitter); 11→10 HEDEF; golden byte-identical; lib coverage = engine/ui/* unit tests)** |
    | hello_world | cd::game::{camera,trigger,particles_event,query} | hello_engine Gameplay panel | ~400L | architect kapısı: 4 gameplay-lib dep | **DONE phase1173 (FOLD — CPU-only "Gameplay Stack Demo" probe in HelloGameplayDemo.hpp; 4 gameplay-lib deps added; golden-neutral)** |
