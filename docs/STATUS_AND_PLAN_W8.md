...
| E      | Clang-tidy rule promotion           | 928, 936, 940, 941, 943, 944, 945, 947, 950, 951, 952, 955, 957, 965, 971, 973-975 | **163 additional rules** swept through bugprone + cert + cppcoreguidelines + hicpp + modernize + performance + portability + readability + concurrency + misc families. **9 → 172** in WarningsAsErrors (**19.1x**). Each promotion preceded by a probe pass; rules with warnings get site-by-site fixes (~60 fix sites across phases 941/943/944/945/946/950/952/971/973/974/975) before promotion. Security-review flagged iterator-invalidation in phase 945's range-for conversion; fixed via snapshot pattern in phase 946. Phase 975 modernize-use-auto sweep ran clang-tidy --fix across 15 source files (35 sites cleaned automatically + 2 reinterpret_cast sites cleaned manually); rule promoted. Phase 976 modernize-use-ranges auto-fix sweep was REVERTED -- clang-tidy --fix corrupted 3 sites (std::upper_bound with heterogeneous comparator, Scene.cpp std::remove pattern) and the rule needs manual per-file conversion -- deferred to Run 26. |
