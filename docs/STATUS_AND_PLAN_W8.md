...
| B      | Per-library live CPU demos          | 919-970  | **36 visible demos shipped** -- DDGI/ReSTIR DI/NRC/vol-fog/clouds/texture-synth/decal/GPU particles/VG/VT/mesh-shader/sheen+clearcoat/SSS/atmo/shafts/LTC/ReSTIR GI/IBL split-sum/camera basis/frustum cull/light CCT/light attenuation/IBL cubemap/CSM splits/cluster grid/motion vector/input axis/ECS world stress/scene serializer/audio tone synth/quaternion slerp/math helpers/cubic bezier/asset registry tag/random PCG32 dist. Every render-tier library in DEPS now has a visible toggle in hello_engine. |
...
| E      | Clang-tidy rule promotion           | 928, 936, 940, 941, 943, 944, 945, 947, 950, 951, 952, 955, 957, 965, 971, 973-975, 978-981 | **164 additional rules** swept through bugprone + cert + cppcoreguidelines + hicpp + modernize + performance + portability + readability + concurrency + misc families. **9 → 173** in WarningsAsErrors (**19.2x**). Each promotion preceded by a probe pass; rules with warnings get site-by-site fixes (~89 fix sites across phases 941/943/944/945/946/950/952/971/973/974/975/978/979/980/981) before promotion. Security-review flagged iterator-invalidation in phase 945's range-for conversion; fixed via snapshot pattern in phase 946. Phase 975 modernize-use-auto sweep ran clang-tidy --fix across 15 source files (37 sites cleaned); rule promoted. Phase 976 modernize-use-ranges auto-fix sweep was REVERTED (clang-tidy --fix corrupted 3 projection-sensitive sites: std::upper_bound + std::remove patterns). Phases 978-981 redid the modernize-use-ranges sweep MANUALLY across 4 batches (32 sites total, including the upper_bound projection-aware conversion + the erase-remove subrange idiom for ranges::remove_if); rule promoted. |
