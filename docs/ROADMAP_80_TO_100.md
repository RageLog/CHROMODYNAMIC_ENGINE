...
## Floor->70 — Wave 1 (phase1267)
58-68 band, 13 libs raised to ~70% (~181 tests, 2 real bugs, 1 feature, 1 pre-existing bug fix).
- light 68->70 (+12), editor 68->70 (+18 EditHistory/Selection/CommandPalette/Hierarchy), volumetric 66->70 (+17 froxel), platform 65->70 (+23 new test file, FIXED pre-existing WebWindow.cpp make_error compile bug), particle_system 65->70 (+12 SoA), vfx_authoring 65->70 (+12), streamer_pool 65->70 (+11 Bresenham), shader_cache 65->70 (+12 CDSC), atmosphere 62->70 (+9, REAL FEATURE: 2nd Hillaire multi-scatter LUT opt-in), imgui_backend 60->70 (+13), denoise 60->70 (+12 a-trous), validator 60->70 (+15 magic-byte), decal 58->70 (+15, honest finding: impl is conservative AABB not 15-axis SAT).
- 2 REAL BUGS (atmosphere multi-scatter LUT, agent's new feature): (1) march ray passes below planet surface -> h<0 -> exp(-h/scale)=+inf -> inf*0=NaN; clamped h>=0. (2) zero-scattering -> transmittance LUT non-finite -> 0*NaN=NaN in L_2nd; finite-guard sun_trans. + platform WebWindow.cpp pre-existing make_error->std::unexpected.
- Gate fixes: 2 compile (platform 2x nodiscard) + 10 WAE (decal 3x member-init, volumetric 2x float-loop->int, imgui ProfilerView 2x min/max, editor EditHistory range-for + HierarchyView/PreferencesStore contains) + 4 test-vs-impl RCA (particle rate*dt<1 spawn math x2 + dt-scaling spawn-timing 0.4/0.4->0.2/0.4, streamer Bresenham per-tick is long-run-avg not hard-cap <=1->2, atmosphere 2 NaN). Gate: build clean -Werror; ctest 337/337; golden BYTE-IDENTICAL; WAE 0.

## Floor->70 — Wave 2+3 (phase1268)
35-56 band, 15 libs raised toward 70 (~342 tests, 2 real bugs already covered in wave-1; deep GPU/decode/CUDA sealed).
- **52-56 → 70**: virtual_textures 56 (+14 PageId/hash/AtlasSlot/FIFO), runtime 55 (+14 DI/watchdog), profile_gpu_marker 55 (+13, GPU-timing sealed), velocity 55 (+16 motion-vector), virtual_geometry 52 (+47 ClusterDAG/dispatch, QEM sealed).
- **35-48 → 70**: ibl_gpu 48 (+12 float_to_half/staging; "0 tests" was stale), audio/scene/texture_streamer 45 (+13/+20/+13 queue/pool/dedup; decode sealed — scene_streamer's glTF load was actually REAL, stale doc), gpu_particles 45 (+14), light_shafts 42 (+16, epipolar sealed), foundation_utils 40 (+50 aggregation contract; 0→50), nrc 40 (+13 strengthened MLP, backends sealed), restir_gi 38 (+32 reservoir math new bin, sample-CS sealed).
- **ui_renderer_webgpu 35 → 57 (HONEST, not 70)**: +12 no-Dawn stub tests; Dawn/WGSL pipeline is a multi-week backend — the no-op path's genuine ceiling is ~57, NOT fake-padded to 70.
- Gate fixes: 8 compile (texture_streamer PathGuard move-deleted vs vector::reserve → move ctor; light_shafts 6x + foundation_utils 1x nodiscard) + 5 WAE (foundation ranges::find, nrc ranges::fill, velocity sqrt2_v, virtual_geometry ClusterDAG dup-include). NO test-vs-impl failures this wave (cleaner). Gate: build clean -Werror; ctest 338/338; golden BYTE-IDENTICAL; WAE 0.

## MARATHON-4 COMPLETE: floor raised to 70. Every lib >=70 EXCEPT ui_renderer_webgpu (57, honest — needs multi-week Dawn pipeline).
