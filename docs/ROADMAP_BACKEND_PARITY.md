...
| **D11** | `begin_parallel_render_pass` not overridden → no D3D12 multithreaded-record speedup (callers fall back to serial). Needs bundle / secondary command-list lane mgmt. | missing | MED | absent; base nullptr `ICommandBuffer.hpp:202` | XL |
