...
namespace CD
{
WindowWindows* WindowWindows::s_window;
WindowWindows::WindowWindows(const WindowInfo&& info) noexcept : Window(std::move(info))
{
  s_window = this;
}
LRESULT WindowWindows::_mProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) noexcept
{
  switch (msg)
  {
    case WM_ERASEBKGND:
...
CD::Errors WindowWindows::Create()
{
  ...
  _hwnd = CreateWindowEx(
      WS_EX_APPWINDOW, CLASS_NAME, _info.WindowTitle.c_str(),
      WS_OVERLAPPED | WS_SYSMENU | WS_CAPTION | WS_MAXIMIZEBOX | WS_MINIMIZEBOX | WS_THICKFRAME,
      _info.positionX, _info.positionY, _info.Width, _info.Height, NULL, NULL, _instance, NULL);
  if (_hwnd == NULL)
  {
    return CD::Errors::Failture;
  }
