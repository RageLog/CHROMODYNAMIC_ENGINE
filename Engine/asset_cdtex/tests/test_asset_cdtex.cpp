...
{
    auto r = cd::asset_cdtex::load("c:/no/such/file.cdtex");
    ASSERT_FALSE(r.has_value());
    EXPECT_EQ(r.error().code, static_cast<std::uint32_t>(cd::asset_cdtex::cdtex_errors::Code::kFileNotFound));
}

TEST(CdTex, EmptyPathReturnsInvalidArgument)
{
    auto r = cd::asset_cdtex::load("");
    ASSERT_FALSE(r.has_value());
    EXPECT_EQ(r.error().code, static_cast<std::uint32_t>(cd::asset_cdtex::cdtex_errors::Code::kInvalidArgument));
}
...
TEST(CdTex, BadVersionReturnsVersionMismatch)
{
    ...
    auto r = cd::asset_cdtex::load(g.path.string());
    ASSERT_FALSE(r.has_value());
    EXPECT_EQ(r.error().code, static_cast<std::uint32_t>(cd::asset_cdtex::cdtex_errors::Code::kVersionMismatch));
}
...
TEST(CdTex, TruncatedPayloadReturnsCorrupt)
{
    ...
    auto r = cd::asset_cdtex::load(g.path.string());
    ASSERT_FALSE(r.has_value());
    EXPECT_EQ(r.error().code, static_cast<std::uint32_t>(cd::asset_cdtex::cdtex_errors::Code::kCorrupt));
}
