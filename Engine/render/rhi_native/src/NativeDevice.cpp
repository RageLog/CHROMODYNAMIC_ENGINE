// =============================================================================
// CHROMODYNAMIC — engine/render/rhi_native/src/NativeDevice.cpp
//
...
NativeDeviceResult try_d3d12_()
{
    ...
    auto d = cd::rhi_d3d12::create_d3d12_device({});
    if (d.has_value() && *d)
    {
        r.device = std::move(*d);
...
{
    NativeDeviceResult r;
    auto d = cd::rhi_metal::create_metal_device({});
    if (d.has_value() && *d)
    {
        r.device = std::move(*d);
...
{
    NativeDeviceResult r;
    auto d = cd::rhi_vulkan::create_vulkan_device({});
    if (d.has_value() && *d)
    {
        r.device = std::move(*d);
