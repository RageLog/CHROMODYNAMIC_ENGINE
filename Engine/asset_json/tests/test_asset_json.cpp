...
TEST(AssetJsonTest, RejectsTrailingGarbage)
{
    ...
    EXPECT_TRUE(
        v.error().code == static_cast<std::uint32_t>(cd::asset_json::json_errors::Code::kBadNumber) ||
        v.error().code == static_cast<std::uint32_t>(cd::asset_json::json_errors::Code::kUnexpectedToken)
    );
}
...
{
    auto v = cd::asset_json::parse("[1,2,3]");
    ASSERT_TRUE(v.has_value());
    cd::core::CVarRegistry reg;
...
{
    auto v = cd::asset_json::parse(R"({"render.msaa":[1,2,3]})");
    ASSERT_TRUE(v.has_value());
    cd::core::CVarRegistry reg;
    auto r = cd::asset_json::from_json(*v, reg);
    ASSERT_FALSE(r.has_value());
...
TEST(CVarBridgeTest, FileRoundTripPreservesState)
{
    ...
    auto sv = cd::asset_json::save_to(path, reg);
...
TEST(CVarBridgeTest, NullValueLeavesCVarUntouched)
{
    ...
    auto v = cd::asset_json::parse(R"({"keep_me": null})");
    ASSERT_TRUE(v.has_value());
    auto r = cd::asset_json::from_json(*v, reg);
